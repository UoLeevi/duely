...
export function Table({ rows: items, columns, headers, className, dense, breakpoint, wrap: wrapOptions, loading, error }) {
  ...
  if (error) {
    ...
    rows = (
      <TableErrorRow message={message} dense={dense} row={isNotWrapped ? 2 : 1} headers={headers} wrapColCount={wrapColCount} wrapColSpanSum={wrapColSpanSum} isNotWrapped={isNotWrapped} />
    );
  } else if (loading) {
    rows = (
      <TableLoadingRow dense={dense} row={isNotWrapped ? 2 : 1} headers={headers} wrapColCount={wrapColCount} wrapColSpanSum={wrapColSpanSum} isNotWrapped={isNotWrapped} />
    );
  } else {
  ...
  return (
    <div className={className} style={{ gridTemplateColumns }}>
      {isNotWrapped && (
        <Fragment>
          <div className="h-8 border-b border-gray-200" style={{ gridArea: `1 / 1 / 2 / -1` }}></div>
          {headers}
        </Fragment>
      )}
      {rows}
...
function TableCell({ children, row, column, span, header, dense, firstRow, lastRow, firstCol, lastCol, isNotWrapped }) {
  ...
  return (
    <div className={className} style={{ gridArea }}>
      <div className="grid text-xs tracking-wide text-gray-500">
        {header}
      </div>
      <div className="relative grid items-center flex-1">
        {children}
      </div>
    </div>
  );
