    ...
    <template v-else>
      ...
      <div class="d-flex flex-column mt-2" v-if="agencies.filter(agency => agency.roles.includes('agent')).length > 0">
        ...
        <div class="d-flex flex-row flex-wrap mb-4">
          <AgencyCard v-for="agency in agencies.filter(agency => agency.roles.includes('agent'))" :key="agency.uuid" :agency="agency" :roles="agency.roles" />
        </div>
      </div>
      <div class="d-flex flex-column mt-2" v-if="agencies.filter(agency => agency.roles.includes('client')).length > 0">
        ...
        <div class="d-flex flex-row flex-wrap mb-4">
          <AgencyCard v-for="agency in agencies.filter(agency => agency.roles.includes('client'))" :key="agency.uuid" :agency="agency" :roles="agency.roles" />
        </div>
      </div>
      ...
      <FloatingBtn text="Create agency" to="/profile/agencies/create-agency" icon="add" />
    </template>
  </section>
</template>
