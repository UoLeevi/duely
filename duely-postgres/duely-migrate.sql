...
-- CALL internal_.drop_auditing_('application_.order_');
-- ALTER TABLE application_.order_ ADD COLUMN sort_key_ real GENERATED ALWAYS AS (CAST(EXTRACT(epoch FROM ordered_at_) AS real)) STORED
-- CALL internal_.setup_auditing_('application_.order_');

DROP FUNCTION operation_.query_resource_(text, jsonb);

CREATE OR REPLACE FUNCTION operation_.query_resource_(_resource_name text, _filter jsonb, _order_by text DEFAULT NULL, _desc boolean DEFAULT 'f', _limit int DEFAULT NULL, _after_id text DEFAULT NULL) RETURNS SETOF jsonb
