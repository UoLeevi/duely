...
CREATE OR REPLACE FUNCTION policy_.anyone_can_query_live_service_variant_(_resource_definition security_.resource_definition_, _resource application_.resource_, _keys text[]) RETURNS text[]
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  IF (
...
PERFORM security_.register_policy_('application_.service_variant_', 'query', 'policy_.anyone_can_query_live_service_variant_');
