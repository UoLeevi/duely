...
export default function DashboardHome() {
  ...
  const columns = [
    // event
    (item: TItem) => <div className="text-sm font-semibold">{item.event}</div>,

    // info
    (item: TItem) => <div className="text-sm">{item.info}</div>,

    // date
    (item: TItem) => <div className="text-sm">{item.date}</div>
  ];
